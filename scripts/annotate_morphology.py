"""Annotate word banks with lightweight prefix/root/suffix hints.

Default mode prints coverage only. Use --write to add/update a "morphology"
field in each word-bank JSON file after manual sampling is acceptable.
"""
import argparse
import json
...
def normalize_word(value):
    return "".join(ch for ch in str(value or "").lower() if "a" <= ch <= "z")
...
def analyze(word):
    ...
    prefix = next((item for item in PREFIXES if word.startswith(item[0]) and len(word) - len(item[0]) >= 4), None)
    prefix_len = len(prefix[0]) if prefix else 0
    suffix = next((item for item in SUFFIXES if word.endswith(item[0]) and len(word) - prefix_len - len(item[0]) >= 3), None)
    suffix_len = len(suffix[0]) if suffix else 0
    base = word[prefix_len:len(word) - suffix_len]
    root = next((item for item in sorted(ROOTS, key=lambda item: len(item[0]), reverse=True) if item[0] in base), None)
    if not root and not suffix and prefix and len(prefix[0]) >= 3:
        return [{"type": "prefix", "text": prefix[0], "meaning": prefix[1]}]
    if not root and not suffix:
        return []

    parts = []
    if prefix and (root or suffix):
        parts.append({"type": "prefix", "text": prefix[0], "meaning": prefix[1]})
    if root:
        parts.append({"type": "root", "text": root[0], "meaning": root[1]})
    if suffix:
        parts.append({"type": "suffix", "text": suffix[0], "meaning": suffix[1]})
    return parts[:3]


def annotate_file(path, write=False):
    data = json.loads(path.read_text(encoding="utf-8-sig"))
    covered = 0
    for item in data:
        parts = analyze(item.get("word"))
        if parts:
            covered += 1
        if write:
            if parts:
...
def main():
    parser = argparse.ArgumentParser(description="Annotate VocabMaster word banks with morphology hints.")
    parser.add_argument("--words-dir", type=Path, default=WORDS_DIR)
    parser.add_argument("--write", action="store_true")
    args = parser.parse_args()

    total = 0
    covered = 0
    for path in sorted(args.words_dir.glob("*.json")):
        if path.name == "metadata.json":
            continue
        file_total, file_covered = annotate_file(path, write=args.write)
        total += file_total
        covered += file_covered
        ratio = file_covered / file_total if file_total else 0
        print(f"{path.name}: {file_covered}/{file_total} ({ratio:.1%})")

    ratio = covered / total if total else 0
    mode = "written" if args.write else "dry-run"
    print(f"Total {mode}: {covered}/{total} ({ratio:.1%})")
